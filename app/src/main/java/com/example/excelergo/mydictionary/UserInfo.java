...
public class UserInfo {
   ...
   public Map<String,String> queryUser(RegisterInfo registerInfo){
       ...
       Cursor cursor=db.query(TABLE_USER,null,"username=? and pwd=?",new String[]{registerInfo.getName(),registerInfo.getPwd()},null,null,null,null);
       if(cursor.getCount()>0) {
   ...
   public String queryFistUser(){
        String name="";
        Cursor cursor=db.rawQuery("select id as _id,username from User where id='1'",null);
        if(cursor.getCount()>0) {
            if(cursor.moveToFirst()) {
